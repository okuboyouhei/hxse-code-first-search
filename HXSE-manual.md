...
### マージモードの絞り込み・ソート・ページネーション（v1.9.0+）

スキーマに `filters`・`sort`・`pagination` のいずれかを追加すると、マージした一覧に絞り込みUI・ソート切り替え・ページャーが付きます。仕組みは外部単一ソース（v1.8.0）と同じで、キャッシュ済みデータへのメモリ内処理です。絞り込み操作でRSSやAPIへの再アクセスは発生しません。

マージ後のアイテムは共通フォーマットに正規化されているため、フィルターの対象フィールドには `title` / `link` / `date` / `excerpt` / `source` を指定します。

```php
$schemas['mixed_news'] = [
    'sources' => [
        [ 'type' => 'wp_query', 'post_type' => 'post', 'label' => 'お知らせ' ],
        [ 'type' => 'rss', 'endpoint' => 'https://zenn.dev/youheiokubo/feed', 'label' => 'Zenn' ],
    ],
    'orderby' => 'date',
    'order'   => 'desc',
    'cache'   => 600,
...
よく使うのが「ソース別の絞り込み」です。`'field' => 'source'` と `'options' => 'auto'` を指定すると、各ソースの `label`（この例では「お知らせ」「Zenn」）が選択肢として自動生成されます。

**注意点：**

- ページネーションはページャーモードのみです（loadmoreは外部ソースと同様に非対応）。
- `limit` はマージ直後・絞り込み前に適用される総件数の上限です。ページネーションを使う場合は通常不要です。
- `filters`・`sort`・`pagination` を指定しなければ、従来どおり全件がテンプレートに渡されます（後方互換）。
...
## 静的サイト（Distan）で使う（v2.0.0+）

Distan などで静的化したサイトでも、検索・絞り込みを動かせます。通常の検索は htmx でサーバーの REST を叩きますが、静的サイトには本番の WordPress がいないため、そのままでは絞り込みが動きません。静的モードは、全件を書き出したうえでブラウザだけで絞り込む方式に切り替わり、サーバーなしで動きます（`file://` で開いても動きます）。

### 使い方

Distan で書き出すときは **自動で切り替わる**ので、特別な設定は要りません（Distan が付ける `X-Distan-Render` ヘッダを HXSE が検知します）。

書き出す前にローカルで挙動を確かめたいときは、スキーマに `'static' => true` を足してください（確認後は外してください。本番は自動判定なので不要です）。
...
- **検索（`search`）・タクソノミー・カスタムフィールド（select系）の絞り込み**がブラウザ内で動きます。
- **ページネーション（v2.1.0+）**が使えます。`pagination` の設定（`per_page` など）をそのまま尊重します。絞り込みで件数が変われば、ページ数も追従します。`mode: 'none'`（または `per_page: 0`）にすれば全件を1ページで表示します。
- **並び替え（v2.2.0+）**が使えます。スキーマの `sort`（`date_desc` / `date_asc` / `title_asc` / `title_desc` / `menu_order`）をそのまま尊重します。絞り込みやページネーションと同時に使え、並び順を変えると1ページ目に戻ります。

### 覚えておくこと

- **有界リスト向けです。** トップページの最新◯件の絞り込みのような、件数が限られた用途に向いています。5年分の記事アーカイブのような大量データには不向きで、全件を書き出すぶん重くなります。500件を超えると、安全のため素のページ送り一覧に切り替わります（`hxse_static_max_items` で調整可）。
- **一部の絞り込みは静的では出ません。** 数値レンジ・投稿年・関連投稿・テーブル表示は、静的モードでは表示されません（並び替えは v2.2.0 で対応しました）。
- **外部ソース（API / RSS など）は、書き出した時点のスナップショット**になります（絞り込みUIは付きません）。
- **記事一覧そのものは WordPress 標準で。** アーカイブの一覧・ページ送りは WordPress 標準や Distan の静的化で作れます。絞り込みUIが要る有界セットだけ HXSE を使う、という住み分けがおすすめです。
...
### v2.2.0
- 静的モードで並び替え（`sort`）に対応しました。`date_desc` / `date_asc` / `title_asc` / `title_desc` / `menu_order` を、絞り込み・ページネーションと合成してクライアント側で並べ替えます（並び順を変えると1ページ目に戻ります）。

### v2.1.0
- 静的モードにクライアント側ページネーションを追加しました。スキーマの `pagination` 設定を尊重し、絞り込みと併用できます（絞り込みで件数が変わればページ数も追従、フィルタ変更で1ページ目へ戻ります）。`mode: 'none'` / `per_page: 0` で全件1ページ表示。
- 静的モードのスクリプトを `wp_footer` から出力するよう修正しました。本文フィルタ（wpautop など）がインラインJSを壊す問題を回避します。

### v2.0.0
- 静的サイト（Distan）向けの「静的モード」を追加しました。静的化したサイトでも、htmx / REST なしで検索・絞り込みが動きます（`X-Distan-Render` ヘッダで自動判定。`'static' => true` で明示も可能）。
- `wp_query` ソースで、検索・タクソノミー・メタ（select系）のクライアント内絞り込みに対応しました。
- `hxse_static_max_items` フィルタ（既定500）を追加。超過時は素のページ送り一覧に縮退します。
- 静的モードでは htmx / hxse.js を読み込みません。レンジ・投稿年・関連・並び替え・テーブル表示は未対応、外部ソースはスナップショットとして書き出します。

### v1.9.0
- マージモード（`sources`）で絞り込み・ソート・ページネーションが使えるようになりました。スキーマに `filters`・`sort`・`pagination` を追加すると有効になります（opt-in）。
- マージ済みアイテムは正規化済みのため、フィルター対象フィールドは `title` / `link` / `date` / `excerpt` / `source` を指定します。`'field' => 'source'` + `'options' => 'auto'` でソース別絞り込みが自動生成されます。
- すべてキャッシュ済みデータへのメモリ内処理です。絞り込み操作で外部への再フェッチは発生しません。

### v1.8.0
- 外部ソース（`api`・`rss`・`xml`）で絞り込みUI・ソート・ページネーションが使えるようになりました（opt-in：スキーマに `filters`・`sort`・`pagination` のいずれかがある場合のみ有効）。
- `select` フィルタータイプを追加。任意のフィールド値との完全一致で絞り込めます。`'options' => 'auto'` でデータから選択肢を自動生成できます。
- `items_key` キーを追加。`{ "events": [...] }` のようにラップされたJSON応答からアイテム配列を取り出せます（ドット記法対応）。
- 同梱デフォルトテンプレート `templates/api.php` を追加しました。
